...
namespace BAREST.Compras
{
    public partial class consultaInsumos : Form
    {
        ...
        {
            Cone.Open();
            string sql = "select descripcion, unidad, cant from Insumo";
            SqlCommand comando = new SqlCommand(sql, Cone);
            SqlDataReader registros = comando.ExecuteReader();
            dataGridView1.Rows.Clear();
            while (registros.Read())
            {
                dataGridView1.Rows.Add(registros["descripcion"].ToString(), registros["unidad"].ToString(), registros["cant"].ToString());
            }
            registros.Close();
            Cone.Close();
        }
